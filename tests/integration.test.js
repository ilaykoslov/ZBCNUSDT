...
function post(path, body) {
    return new Promise((resolve, reject) => {
        ...
        req.on('error', reject);
        req.setTimeout(5000, function(){ this.destroy(); reject(new Error('Timeout')); });
        req.write(data);
        req.end();
    });
}
...
async function runTests() {
    ...
    await test('Her iki sembol için API çalışıyor', async () => {
        const [z, p] = await Promise.all([
            get('/api/all?symbol=ZBCNUSDT'),
            get('/api/all?symbol=PROSUSDT')
        ]);
        if (!z.ticker || !p.ticker) throw new Error('One of the symbols failed');
    });
